...
interface AutoTranslated {
    ...
    enum class StringStyle(val modify: (String) -> String) {
        // TODO: make a toCamelCase function
        CamelCase({ str ->
            str.toSnakeCase().split("_").joinToString("") { it.capitalize() }.replaceFirstChar(Char::lowercaseChar)
        }),
    ...
    val stringStyle: StringStyle get() = StringStyle.snake_case
    ...
    fun autoButton(id: String, buttonBuilder: ButtonBuilder.() -> Unit) = entryBuilder.button {
        val translation = makeTranslation(id)
        this.description = "$translation.desc"
    ...
    context(entryBuilder: EntriesBuilder)
    fun autoSeparator(id: String, separatorBuilder: SeparatorBuilder.() -> Unit = {}) = entryBuilder.separator {
        this.translation = makeTranslation(id)
        separatorBuilder()
    }
