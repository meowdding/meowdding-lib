...
tasks.named("build") {
    ...
    val files = tasks.named("jar").map { it.outputs.files }
    inputs.properties(
...
dependencies {
    ...
    api(versionedCatalog["meowdding.remote.repo"]) // makes it transitive -> consuming mods don't need to add a dependency on it
    ...
    includeImplementation(versionedCatalog["keval"])
